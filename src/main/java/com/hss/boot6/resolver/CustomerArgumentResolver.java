package com.hss.boot6.resolver;

import com.hss.boot6.ParameterModel;
        ...
        implements HandlerMethodArgumentResolver
{
    ...
    /**
     *
     * 该方法作为装载参数逻辑的入口，我们从MethodParameter对象内获取了参数的名称，
     * 4根据该名称检查Model内是否存在该名称的值，如果存在则直接使用并返回，
     * 反则需要从ParameterMap内获取对应该参数名称的值返回。
    ...
    /**
     *该方法首先实例化了一个MethodParameter类型的空对象，然后通过getPrefixParameterMap获取PathVariables、
     * ParameterMap内前缀为MethodParameter名称的请求参数列表，遍历列表对应设置
     * object内的字段，用于完成参数的装载，在装载过程中，我这里分别根据Collection、List、Array、Single类型进行了处理
     * （注意：这里需要根据项目需求进行调整装载类型）。
